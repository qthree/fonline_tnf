...
struct GameOptions
{
    ...
    Sprite**     ( *GetDrawingSprites )( uint & count );                            // какие же они уебки
...
struct NpcPlane
{
    ...
    uint      GetChildIndex( NpcPlane* child ) { uint index = 0;
                                                 for( NpcPlane* child_ = this; child_; index++ ) { if( child_ == child ) break;
                                                                                                   else child_ = child_->ChildPlane; } return index; }
    uint GetChildsCount() { uint      count = 0;
...
// наркоман штоле?
    ...
    int  GetDeteriorationProc() { int val = GetDeterioration() * 100 / MAX_DETERIORATION;
...
{
    void*  Surface;             // 0x0
    float  SurfaceUV[ 4 ];      // 0x4 //0x8 //0xC //0x10
    uint16 Width;               // 0x14
    uint16 Height;              // 0x16
    int16  OffsX;               // 0x18
    int16  OffsY;               // 0x1A
...
{
    // Ordering
    int     DrawOrderType;  // 0x0	// 0..4 - flat, 5..9 - normal
    uint    DrawOrderPos;   // 0x4
    uint    TreeIndex;      // 0x8

    // Sprite information, pass to GetSpriteInfo
    uint    SprId;          // 0xC
    uint*   PSprId;         // 0x10		// If PSprId == NULL than used SprId

    // Positions
    int     HexX, HexY;     // 0x14	0x18
    int     ScrX, ScrY;     // 0x1C	0x20
    int16*  OffsX, * OffsY; // 0x24	0x28
...
{
    if( !Game->ClientMap || hexX >= Game->ClientMapWidth || hexY >= Game->ClientMapHeight )
        return NULL;
